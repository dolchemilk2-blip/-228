...
const HIST_KEYS = ['timing', 'fxLine', 'fxVoice', 'voiceGains', 'gains', 'tempo', 'stage'];
...
function histApply(snap) {
  ...
  for (const k of HIST_KEYS) S[k] = next[k] ?? (k === 'tempo' ? 1 : k === 'stage' ? null : {});
  saveEdits();
  if (!S.result) return;
  ...
  const stage = JSON.stringify(prev.stage) !== JSON.stringify(next.stage);           // мизансцена: места, движение, стерео
  const layout = histDiff(prev.timing, next.timing).length || prev.tempo !== next.tempo || stage;
  if (stage && typeof renderSpace === 'function') renderSpace();
